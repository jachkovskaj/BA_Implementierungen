...
def _load_best_metrics_for_reduction(
    base_dir,
    ...
    """
    Lädt für eine Reduktionsmethode (CBIR / RegMix) die Metriken
    ...
    Besonderheit CBIR:
      - Study-Ordner heißen zusätzlich ..._CONF_* (z.B. _CONF_80/_CONF_85/_CONF_90/_CONF_95),
        Anzahl/Existenz pro Seed beliebig -> wird via glob() gefunden.
    """

    reduction = str(reduction_name).strip()
    ...
    if model_nummer == 1:
        ...
        model_label = f"{model_nummer}.{submodel_nummer}"

    elif model_nummer == 2:
        plan_name = "LHS"
        split_fix = "KS"
        submodel_nummer = None
        model_label = f"{model_nummer}"
    ...
    for seed in seeds:
        # -------- Prefix (ohne _CONF_*) bauen --------
        if model_nummer == 1:
            base_prefix = (
                f"Study_15_10_2025_{plan_name}_Modell_{model_nummer}.{submodel_nummer}_"
                f"{plan_name}_{split_fix}_{reduction}_Holdout_seed_{seed}"
            )
        else:
            # Achtung: in deinen Teil-3 Namen steht vorne oft "Study_15_10_2025_Halton_Modell_2_..."
            # obwohl plan_name = LHS ist. Daher hier bewusst so gebaut:
            base_prefix = (
                f"Study_15_10_2025_{plan_name}_Modell_{model_nummer}_"
                f"{plan_name}_{split_fix}_{reduction}_Holdout_seed_{seed}"
            )

        # -------- Candidate-Ordner bestimmen --------
        if reduction_upper == "CBIR":
            candidates = sorted(base.glob(base_prefix + "_CONF_*"))
            if not candidates:
                print(f"[Hinweis] Keine CBIR-CONF Studies gefunden für Seed {seed}: {base_prefix + '_CONF_*'}")
        else:
            candidates = [base / base_prefix]
            if not candidates[0].exists():
                print(f"[Hinweis] Study-Ordner fehlt: {candidates[0]}")

        # -------- Jede gefundene Study auswerten --------
        for study_dir in candidates:
            ...
            # Metrics-File (Reeval, eigene Testdaten)
            if model_nummer == 1:
                metrics_file = (
                    study_dir / "metrics" /
                    f"metrics_{trial_num}__Reeval_{plan_name}_Modell_{model_nummer}.{submodel_nummer}.csv"
                )
            else:
                metrics_file = (
                    study_dir / "metrics" /
                    f"metrics_{trial_num}__Reeval_{plan_name}_Modell_{model_nummer}.csv"
                )
            ...
            if not all(col in lower_map for col in needed):
                print(
                    f"[Hinweis] Metrics-Datei {metrics_file} ohne (dataset/rmse/r2) "
                    f"→ Spalten: {list(df_metrics.columns)}"
                )
                continue
    ...
    if not metrics_list:
        raise RuntimeError(f"Keine gültigen Metrics für {reduction}, Modell {model_label} gefunden.")

    df_all = pd.concat(metrics_list, axis=0)

    # Mittelwert / Std über (Seeds * ggf. CONF-Varianten)
...
def plots(model_nummer, submodel_nummer=None, seeds=(0, 42, 999), base_dir="Ergebnisse_Teil_3"):
    # Plan-Fixierung + Label
    if model_nummer == 1:
        if submodel_nummer is None:
            raise ValueError("Für Modell 1 muss submodel_nummer (1, 2 oder 3) angegeben werden.")
        fixed_plan = "Halton"
    ...
    rmse_train_vals = np.array([data_rmse_mean[r][0] for r in reductions])
    rmse_val_vals   = np.array([data_rmse_mean[r][1] for r in reductions])
    rmse_test_vals  = np.array([data_rmse_mean[r][2] for r in reductions])

    rmse_train_std  = np.nan_to_num(np.array([data_rmse_std[r][0] for r in reductions]))
    rmse_val_std    = np.nan_to_num(np.array([data_rmse_std[r][1] for r in reductions]))
    rmse_test_std   = np.nan_to_num(np.array([data_rmse_std[r][2] for r in reductions]))

    r2_train_vals = np.array([data_r2_mean[r][0] for r in reductions])
    r2_val_vals   = np.array([data_r2_mean[r][1] for r in reductions])
    r2_test_vals  = np.array([data_r2_mean[r][2] for r in reductions])

    r2_train_std  = np.nan_to_num(np.array([data_r2_std[r][0] for r in reductions]))
    r2_val_std    = np.nan_to_num(np.array([data_r2_std[r][1] for r in reductions]))
    r2_test_std   = np.nan_to_num(np.array([data_r2_std[r][2] for r in reductions]))

    modell_label = f"{model_nummer}" if submodel_nummer is None else f"{model_nummer}.{submodel_nummer}"

    out_dir = (
        f"Ergebnisplots_Visualization_Metriken_Datenreduktion_eigene_Daten/"
        f"Plots_Vergleich_{fixed_plan}_Modell_eigene_Testdaten_{modell_label}"
    )
    ...
    ax.set_ylabel("RMSE")
    ax.set_title(f"RMSE von Train / Val / Test – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_1_RMSE_Train_Val_Test.png"))
    plt.close(fig)
    ...
    ax.set_xticklabels(reductions, rotation=90)
    ax.set_ylabel("R²")
    ax.set_title(f"R² von Train / Val / Test – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_2_R2_Train_Val_Test.png"))
    plt.close(fig)
    ...
    ax_r2.set_ylabel("R²")
    ax_rmse.set_title(f"RMSE & R² (Train / Validation / Test) – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_3_RMSE_R2_Train_Val_Test.png"))
    plt.close(fig)
    ...
    ax.set_title(f"RMSE (Test) – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_4_RMSE_nur_Test.png"))
    plt.close(fig)
    ...
    ax.set_title(f"R² (Test) – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_5_R2_nur_Test.png"))
    plt.close(fig)
    ...
    ax_rmse_test.set_title(f"RMSE & R² (Test) – {fixed_plan}, Modell {modell_label} (eigene Testdaten)")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"{fixed_plan}_Modell_eigene_Testdaten_{modell_label}_Plot_6_RMSE_R2_Test.png"))
    plt.close(fig)

    print(f"[OK] Alle Vergleichsplots (CBIR vs RegMix) für {fixed_plan}, Modell {modell_label} in {out_dir} gespeichert.")


# =========================
# Aufrufe
# =========================
plots(model_nummer=1, submodel_nummer=1, base_dir="Ergebnisse_Teil_3")
plots(model_nummer=1, submodel_nummer=2, base_dir="Ergebnisse_Teil_3")
plots(model_nummer=1, submodel_nummer=3, base_dir="Ergebnisse_Teil_3")

plots(model_nummer=2, base_dir="Ergebnisse_Teil_3")