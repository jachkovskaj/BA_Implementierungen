...
def parse_study_name(study_name: str):
    ...
    Modell 2: Study_15_10_2025_LHS_Modell_2_{split}_Holdout_seed_{seed}
            (optional auch Modell_2.{sub}, falls du es doch hast)
    """
    rx = re.compile(
        r"^Study_15_10_2025_"
        r"(?P<plan>Halton|LHS)_"
        r"Modell_(?P<model>[12])"
        r"(?:\.(?P<sub>[123]))?_"      # <-- sub OPTIONAL
        r"(?P<split>KS|DUPLEX|SPlit|SPXY)_Holdout_seed_(?P<seed>0|42|999)$",
        re.I
    )
    m = rx.match(study_name)
    if not m:
        return None, None, None, None, None

    plan  = m.group("plan")
    model = int(m.group("model"))
    sub   = m.group("sub")   # None möglich (typisch Modell 2)
    split = m.group("split")
    seed  = int(m.group("seed"))

    # Safety: Modell 1 MUSS sub haben
    if model == 1 and sub is None:
        return None, None, None, None, None

    return plan, model, sub, split, seed
...
def load_normalization_and_holdout(plan: str, model_nummer: int, sub: str = None):
    ...
    """
    if model_nummer == 1:
        holdout_file = DATA / f"Holdout_Halton_fixed_Modell_1.xlsx"
        pool_file    = DATA / f"Pool_Halton_Modell_1.xlsx"
        if sub is None:
            raise ValueError("Für Modell 1 muss sub (1/2/3) gesetzt sein.")
    else:
        holdout_file = DATA / f"Holdout_LHS_fixed_Modell_2.xlsx"
        pool_file    = DATA / f"Pool_LHS_Modell_2.xlsx"
...
def reevaluate(model_nummer: int, base_dir: str = "Ergebnisse_Teil_2"):
    ...
    for s in sorted(root.iterdir()):
        if not s.is_dir():
            continue
        plan, model_from_name, sub, split, seed = parse_study_name(s.name)
        if plan is None:
            continue
        if model_from_name != model_nummer:
            continue
...
def _reevaluate_study(study: Path, load_model, model_nummer: int):
    ...
    plan, model_from_name, sub, split, seed = parse_study_name(study_name)
    if plan is None:
        print(f"[SKIP] Name passt nicht: {study_name}")
        return

    # Safety: nur Studien, die wirklich zu model_nummer passen
    if model_from_name != model_nummer:
        return

    # Plan-Fixierung: Modell 1 -> Halton, Modell 2 -> LHS
    test_plan = "Halton" if model_nummer == 1 else "LHS"

    # Safety: Plan muss dazu passen
    if plan.lower() != test_plan.lower():
        print(f"[SKIP] Plan passt nicht (erwartet {test_plan}): {study_name}")
        return

    # Output getrennt nach Split UND Seed (optional, aber super praktisch)
    OUT = (Path("Ergebnisse_Summary_Reeval_Datenaufteilung")
           / f"Reeval_{test_plan}_Modell_{model_nummer}"
           / f"Split_{split}"
           / f"seed_{seed}")
    ...
    SUMMARY_XLSX = OUT / f"summary_{test_plan}_Modell_{model_nummer}_Split_{split}_seed_{seed}.xlsx"

    # WICHTIG: test_plan verwenden, nicht plan
    X_hold_norm, y_true, norm_params = load_normalization_and_holdout(test_plan, model_nummer, sub)
    ...
    for mdl in sorted(models_dir.glob("*.keras")):
        ...
        if met.is_dir():
            ...
            if c:
                upd_metrics(c[0], r, R, test_plan, model_nummer, sub, trial)

        tx = study / f"Trial_{trial}.xlsx"
        if not tx.exists():
            tx = (study / "metrics") / f"Trial_{trial}.xlsx"
        if tx.exists():
            upd_trial(tx, y_true, y_pred, test_plan, model_nummer, sub, trial)
