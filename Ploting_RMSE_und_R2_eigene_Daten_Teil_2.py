...
def _load_best_metrics_for_split(
    base_dir,
    split_name,
    model_nummer,
    ...
    Fixierung:
      - Modell 1 -> plan_name = "Halton"
      - Modell 2 -> plan_name = "LHS"
    """
    if model_nummer == 1:
        plan_name = "Halton"
    ...
    for seed in seeds:
        # Study-Name bauen (Split variiert!)
        if model_nummer == 1:
            study_name = (
        ...
        # Metrics-File (Reeval, eigene Testdaten) – dein Namensschema beibehalten
        if model_nummer == 1:
            metrics_file = (
                study_dir / "metrics" /
    ...
    if not metrics_list:
        raise RuntimeError(f"Keine gültigen Metrics für Split {split_name}, Modell {model_label} gefunden.")
...
def plots(model_nummer, submodel_nummer=None, seeds=(0, 42, 999), base_dir="Ergebnisse_Teil_2"):
    ...
    out_dir = (
        f"Ergebnisplots_Visualization_Metriken_Aufteilungsmethode_eigene_Daten/"
