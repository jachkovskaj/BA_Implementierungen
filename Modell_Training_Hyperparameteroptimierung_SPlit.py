...
def split_pool_with_split_r(
    pool_df_in,
    pool_df_out,
    seed,
    val_ratio=0.111111,
    r_script="SPlit_data.r",
    rscript_bin="Rscript"
):
    # eindeutiger Basisname, falls du ihn später loggen willst
    base = f"TEMP_SPLIT_POOL_{os.getpid()}_{uuid.uuid4().hex}"

    y_name = getattr(pool_df_out, "name", None) or "y"
    tmp_df = pd.concat(
        [
            pool_df_in.reset_index(drop=True),
            pool_df_out.reset_index(drop=True).rename(y_name)
        ],
        axis=1
    )
    ...
    with tempfile.TemporaryDirectory(prefix="split_run_") as td:
        td = Path(td)

        # alles nur in diesem temp-ordner!
        (td / f"{base}.xlsx").write_bytes(b"")  # optionaler "touch", nicht nötig
        tmp_df.to_excel(td / f"{base}.xlsx", index=False)

        (td / "temp_filename.txt").write_text(base)
        (td / "val_ratio.txt").write_text(str(val_ratio))
        (td / "seed.txt").write_text(str(seed))

        # R im temp-ordner laufen lassen, damit es genau dort liest/schreibt
        res = subprocess.run(
        ...
        train = pd.read_excel(td / f"{base}_train.xlsx")
        val   = pd.read_excel(td / f"{base}_val.xlsx")
...
def data():
    ...
    # Original-Spaltennamen merken (Input + Output)
    input_cols = list(pool_df.columns[:10])
    target_col = pool_df.columns[-4]  # deine Output-Spalte

    # optional: speichern in hpoptimize, falls du es später brauchst
    hpoptimize.input_cols = input_cols
    hpoptimize.target_col = str(target_col)
    ...
    # !!! Spaltennamen nach Normalisierung explizit wieder setzen !!!
    pool_df_in.columns = input_cols
    pool_df_out.name = str(target_col)

    holdout_df_in.columns = input_cols
    holdout_df_out.name = str(target_col)

    # SPlit Aufteilung (über R-Skript)
    x_train, x_val, y_train, y_val = split_pool_with_split_r(
        pool_df_in,
        pool_df_out,
        seed=hpoptimize.seed,
        val_ratio=0.111111,
        r_script="SPlit_data.r",
        rscript_bin="/opt/homebrew/bin/Rscript"  # ggf. anpassen (voller Pfad), falls Rscript nicht im PATH ist
    )
...
if __name__ == '__main__':
    ...
    study_name = "Study_15_10_2025_Halton_Modell_1.1_SPlit_Holdout_seed_42"
