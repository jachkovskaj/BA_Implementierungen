...
def parse_plan_and_submodel(study_name: str, model_nummer: int):
    """
    Extrahiert Plan (Halton/LHS/Sobol/Taguchi).
    Für Modell 1 zusätzlich Submodell-Index '1','2','3' aus z.B.:

    Study_15_10_2025_Halton_Modell_1.3_KS_Holdout_seed_0
        -> plan='Halton', sub='3'

    Für Modell 2 gibt es kein Submodell → sub = None
    """
...
def _reevaluate_study(study: Path, load_model, model_nummer: int):
    ...
    OUT   = Path("Ergebnisse_Summary_Reeval") / f"Reeval_{plan}_Modell_{model_nummer}"
...
def reevaluate(model_nummer: int, base_dir: str = "Ergebnisse_Teil_1"):
    """
    Reevaluierung für Modell 1 oder 2.

    Beispiel:
        reevaluate(1)  -> alle Modell_1.*-Studien
        reevaluate(2)  -> alle Modell_2-Studien
    """
    ...
    # Studien-Pattern je Modell
    patt = re.compile(rf"(Halton|LHS|Sobol|Taguchi).*Modell_{model_nummer}(\.|_)", re.I)

    for s in sorted(root.iterdir()):
        if s.is_dir() and patt.search(s.name):
